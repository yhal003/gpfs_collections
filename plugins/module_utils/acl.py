...
class NFSv4_PermDict:
    """
     Represents a list of NFS
    """
    @staticmethod
    def yes_all():
        permissions = { permission: True
                        for permission in
                        NFSv4_PermDict.PERMISSIONS }
        return NFSv4_PermDict(permissions)

    @staticmethod
    def yes_except(permissions):
        result = NFSv4_PermDict.yes_all()
    ...
    def no_except(permissions):
        return NFSv4_PermDict.yes_only(permissions)

    @staticmethod
    def no_only(permissions):
        return NFSv4_PermDict.yes_except(permissions)

    @staticmethod
    def read(text):
        perms = text.split()
        result = NFSv4_PermDict({})
        for p in perms:
            m = re.match(r"\((.)\)(.*)", p)
            allow = m.group(1)
            perm_name = m.group(2)
    ...
    def __getitem__(self, permission):
        if permission in NFSv4_PermDict.PERMISSIONS:
    ...
    def __setitem__(self, permission, allow):
        ...
        if permission not in NFSv4_PermDict.PERMISSIONS:
            raise IndexError(f"Unknown permission {permission}")
        self.permissions[permission] = allow

    def __repr__(self):
        result = " "
        for p in NFSv4_PermDict.PERMISSIONS:
...
class NFSv4_PermSpec:
    ...
    def __init__(self,
        ...
        if not flags.issubset(NFSv4_PermSpec.FLAGS):
            raise NFSv4_SyntaxError(f"flags must be any of {NFSv4_PermSpec.FLAGS}")
        if spec_type not in NFSv4_PermSpec.SPEC_TYPES:
            raise NFSv4_SyntaxError("spec_type must be allow or deny")
        if aud_type not in NFSv4_PermSpec.AUD_TYPES:
            raise NFSv4_SyntaxError(f"aud_type must be on of {NFSv4_PermSpec.AUD_TYPES}")
        if not re.match("[r-][w-][x-][c-]", unix_bits):
            raise NFSv4_SyntaxError(f"unix bits {unix_bits} are not valid")
...
class NFSv4_ACL:
    ...
    def read(string_repr: str):
        ...
        for entry in string_repr.split("\n\n"):
            if entry.strip() == "":
                ...
                perms = NFSv4_PermDict.read(" ".join(content_lines[1:]))
    ...
    def from_ansible(ansible_list):
        ...
        for entry in ansible_list:
            if "spec" not in entry:
                raise NFSv4_SyntaxError("all ACL entries must include spec")
            spec = NFSv4_PermSpec.read(entry["spec"])
            allowed_perms = set(["yes_except", "yes_only",
                                "no_except", "no_only"])
            if len(allowed_perms - set(entry.keys())) != 3:
                raise NFSv4_SyntaxError(f"one and only one of {allowed_perms} is required")
            if "yes_except" in entry:
                perm = NFSv4_PermDict.yes_except(entry["yes_except"])
            if "yes_only" in entry:
                perm = NFSv4_PermDict.yes_only(entry["yes_only"])
            if "no_except" in entry:
                perm = NFSv4_PermDict.no_except(entry["no_except"])
            if "no_only" in entry:
                perm = NFSv4_PermDict.no_only(entry["no_only"])
            result.append(spec, perm)
        return result

    @staticmethod
    def mmgetacl(filename):
        acl_proc = subprocess.run([join(BINARY_PATH, "mmgetacl"),
                                   "-k", "nfs4", filename],
                                   check = True,
    ...
    def mmputacl(filename, acl):
        acl_proc = subprocess.run([join(BINARY_PATH, "mmputacl"),
                                    filename],
                                   check = False,
    ...
    def append(self, spec: NFSv4_PermSpec, perms: NFSv4_PermDict):
