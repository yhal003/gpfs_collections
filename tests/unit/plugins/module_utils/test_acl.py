...
def test_all_yes():
    ...
    assert perm_list["READ/LIST"]
    assert perm_list["CHOWN"]
...
"""
def test_read_entry():
    acl = NFSv4_PermDict.read(ENTRY1)
...
def test_equal():
    acl1 = NFSv4_PermDict.yes_all()
    acl2 = NFSv4_PermDict.yes_all()
    assert acl1 == acl2
...
def test_yes_except_nothing():
    acl = NFSv4_PermDict.yes_except([])
    expected = NFSv4_PermDict.yes_all()
    assert acl == expected

def test_yes_except_read():
    acl = NFSv4_PermDict.yes_except(["READ/LIST"])
    expected = NFSv4_PermDict.yes_all()
    expected["READ/LIST"] = False
    assert acl == expected

def test_yes_except_two():
    acl = NFSv4_PermDict.yes_except(["READ/LIST", "CHOWN"])
    expected = NFSv4_PermDict.yes_all()
    expected["READ/LIST"] = False
...
def test_diff_order_doesnt_matter():
    ...
    plist1 = NFSv4_PermDict.yes_all()

    spec2 = NFSv4_PermSpec("user", "john", "rwxc", "allow", set([]))
    plist2 = NFSv4_PermDict.no_all()
...
def test_ansible_no_spec():
    with pytest.raises(NFSv4_SyntaxError):
        ansible_list = [
         {
            "yes_only": ["WRITE"]
         }
...
def test_ansible_simple():
    ...
    assert len(acl) == 1
    (spec, perms) = acl[0]
    assert perms == NFSv4_PermDict.yes_all()
    assert spec.spec_type == "allow"
